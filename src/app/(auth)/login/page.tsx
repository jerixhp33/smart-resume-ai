...
export default function LoginPage() {
  ...
  const { register: registerMagic, handleSubmit: handleMagicSubmit, formState: { errors: magicErrors, isSubmitting: magicSubmitting } } = useForm<MagicForm>({
  ...
  async function onMagicLink(data: MagicForm) {
      ...
      <div className="bg-card rounded-2xl border border-border p-8 shadow-sm">
        ...
        <div className="flex rounded-lg bg-muted p-1 mb-5">
          <button
            ...
            className={`flex-1 py-1.5 text-sm font-medium rounded-md transition-all ${mode === 'password' ? 'bg-card shadow-sm text-foreground' : 'text-muted-foreground'}`}
          >
            Password
          </button>
          <button
            onClick={() => setMode('magic')}
            className={`flex-1 py-1.5 text-sm font-medium rounded-md transition-all ${mode === 'magic' ? 'bg-card shadow-sm text-foreground' : 'text-muted-foreground'}`}
          >
            Magic Link
          </button>
        </div>
        ...
        {mode === 'password' ? (
          ...
          <div className="text-center py-4">
            ...
            <p className="text-xs text-muted-foreground mt-1">We sent a sign-in link to your email.</p>
            <button onClick={() => setMagicSent(false)} className="mt-4 text-xs text-primary hover:underline">
              Try a different email
            </button>
          </div>
        ) : (
          <form onSubmit={handleMagicSubmit(onMagicLink)} className="space-y-4" noValidate>
            <Input
              label="Email"
              type="email"
              placeholder="you@example.com"
              ...
              {...registerMagic('email')}
            />
            <Button type="submit" className="w-full h-11" loading={magicSubmitting}>
              Send magic link
              <ArrowRight className="h-4 w-4" />
            </Button>
          </form>
